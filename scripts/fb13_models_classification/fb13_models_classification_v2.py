...
train_path = 'data/fb13/FB13-train.pkl'
valid_path = 'data/fb13/FB13-valid.pkl'
test_path = 'data/fb13/FB13-test.pkl'
...
# def adagrad(param, rate, epsilon, gradient, updates, param_squared_gradients):
cmd_adagrad = ('./learn_parameters.py --seed=%d --strategy=%s --totepochs=%d --test_all=10 --lr=%f --name=fb13_classification/fb13_%s_%d '
                ' --train=%s --valid=%s --test=%s --nbatches=%d --no_rescaling --filtered --classification'
                ' --op=%s --sim=%s --ndim=%d --nhid=%d --margin=%d ' # varying params
                ' > logs/fb13_models_classification/fb13_classification.%s.%s.%d.%d.%d.%d.log 2>&1')
