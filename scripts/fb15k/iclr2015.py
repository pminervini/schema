...
cmd_adagrad = './learn_parameters.py --op=%s --sim=%s --strategy=%s --totepochs=%d --test_all=1 --lr=%f --ndim=%d --l2_param=%f --name=fb15k_%d  --train=%s --valid=%s --test=%s --nbatches=%d > logs/fb15k.%d.log 2>&1'
                ...
                for ndim in ndim_set:
                    for lr in lr_set:
                        for l2_regularizer in l2_regularizer_set:
                            print(cmd_adagrad % (op, sim, method, epochs, lr, ndim, l2_regularizer, c, train_path, valid_path, test_path, nbatches, c))
                            c += 1
