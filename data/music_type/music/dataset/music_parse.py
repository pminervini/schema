...
for datatyp in ['train', 'valid', 'test']:
    f = open(datapath + 'music_mte5_%s.nt' % datatyp, 'r')
    dat = f.readlines()
    f.close()
    for i in dat:
...
f = open('./data/music_entity2idx.pkl', 'w')
g = open('./data/music_idx2entity.pkl', 'w')
...
for datatyp in ['train', 'valid', 'test']:
    ...
    if 'data' not in os.listdir('../'):
        os.mkdir('../data')
    f = open('./data/music-%s-lhs.pkl' % datatyp, 'w')
    g = open('./data/music-%s-rhs.pkl' % datatyp, 'w')
    h = open('./data/music-%s-rel.pkl' % datatyp, 'w')
