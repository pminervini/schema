...
def learn(state):
    ...
    for epoch_count in range(1, state.totepochs + 1):
        ...
        if domain_range is None or state.is_dr_standard_learning:
            trainln = util.create_random_mat(trainl.shape, trainln_arange)
            trainrn = util.create_random_mat(trainr.shape, trainrn_arange)
        else:
            # Constrain the creation of the trainln and trainrn random matrices.

            # Find relation indices:
            (rows, _) = traino.nonzero()
            rel_idxs = np.sort(np.unique(rows))

            # For each relation index, retrieve the corresponding domains and sets
            domain_values = {rel:relation2domain[rel] for rel in rel_idxs}
            range_values = {rel:relation2range[rel] for rel in rel_idxs}

            trainln = util.create_random_submat_hash(trainl.shape, rows, domain_values, listidx=trainln_arange)
            trainrn = util.create_random_submat_hash(trainr.shape, rows, range_values, listidx=trainrn_arange)
        ...
        if (state.test_all is not None) and ((epoch_count % state.test_all) == 0):
            ...
            if dataset.has_valid is True and not state.is_classification:
                ...
                if domain_range is not None:
                    resvalid = evaluation.FilteredRankingScoreIdx_DR(ranklfunc, rankrfunc, validlidx, validridx, validoidx,
                                                                        relation2domain, relation2range, true_triples=None)
                    valid_summary = evaluation.ranking_summary(resvalid, idxo=validoidx, tag='DR raw valid')
                    state.valid = np.mean(resvalid[0] + resvalid[1])

                    if (state.filtered):
                        resvalid_filtered = evaluation.FilteredRankingScoreIdx_DR(ranklfunc, rankrfunc, validlidx, validridx, validoidx,
                                                                                    relation2domain, relation2range, true_triples=true_triples)
                        valid_summary_filtered = evaluation.ranking_summary(resvalid_filtered, idxo=validoidx, tag='DR filtered valid')
            ...
            if dataset.has_test is True and not state.is_classification:
                ...
                if domain_range is not None:
                    restest = evaluation.FilteredRankingScoreIdx_DR(ranklfunc, rankrfunc, testlidx, testridx, testoidx,
                                                                    relation2domain, relation2range, true_triples=None)
                    test_summary = evaluation.ranking_summary(restest, idxo=testoidx, tag='DR raw test')
                    state.test = np.mean(restest[0] + restest[1])

                    if (state.filtered):
                        restest_filtered = evaluation.FilteredRankingScoreIdx_DR(ranklfunc, rankrfunc, testlidx, testridx, testoidx,
                                                                                    relation2domain, relation2range, true_triples=true_triples)
                        test_summary_filtered = evaluation.ranking_summary(restest_filtered, idxo=testoidx, tag='DR filtered test')
...
def launch(op='TransE', simfn=similarity.dot, ndim=20, nhid=20, Nsyn=None,
        ...
        domain_range_pkl=None, is_dr_standard_learning=False):
    ...
    state.is_dr_standard_learning = is_dr_standard_learning
...
def main(argv):
    ...
    usage_str = ("""Usage: %s [-h]
                    ...
                    [--classification] [--domain_range=<file.pkl>] [--dr_standard_learning]
                    """ % (sys.argv[0]))
    ...
    try:
        opts, args = getopt.getopt(argv, 'h', [ 'name=', 'train=', 'valid=', 'test=', 'use_db',
                                                ...
                                                'classification', 'domain_range=', 'dr_standard_learning' ])
    ...
    for opt, arg in opts:
        ...
        if opt == '-h':
            ...
            logging.info('\t--dr_standard_learning (default: %s)' % (is_dr_standard_learning))
        ...
        elif opt == '--dr_standard_learning':
            is_dr_standard_learning = True
    ...
    launch(op=op, simfn=simfn, method=method, seed=seed, totepochs=totepochs,
            ...
            domain_range_pkl=domain_range_pkl, is_dr_standard_learning=is_dr_standard_learning)
